...
class ScriptGenerator:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            "You are an expert children's content creator and cinematic prompt engineer writing animation prompts for YouTube Shorts.\n"
            ...
            f"ONE-SHOT PROMPT FRAMEWORK DOCUMENT:\n{framework_text}\n\n"
            "ADDITIONAL SYSTEM RULES:\n"
            "1. Total Video Duration: EXACTLY 10.0 seconds total.\n"
            "2. ONE-SHOT CONTINUOUS SEQUENCE: NO camera cut scenes, NO multi-shot headers, NO abrupt camera jumps. The entire 10-second video is ONE single smooth continuous shot.\n"
            "3. Camera & Rendering Specs: Prompt MUST open with: 'Single continuous one-shot video clip, Masterpiece 3D Pixar feature-film animation quality, Octane Render 3D aesthetic, [Shot size], [Lens], [Camera Angle], [Camera Movement], warm studio key lighting, soft rim lighting highlights along coat/fur, subsurface scattering (SSS) shader, soft depth of field — '\n"
            "4. Mascot Binding: Every mascot reference MUST state: '[Mascot], the mascot character from image 1 and image 2...'\n"
            "5. EXPLICIT UNCOMPRESSED EVENT DETAIL: NEVER use vague summaries like 'sequentially', 'numbers 1 to 10 pop up', or 'etc.'. EVERY SINGLE visual element must be explicitly named with timestamps (e.g., 'At 00:02 3D red ball pops up. At 00:05 3D blue ball pops up. At 00:08 3D pink ball pops up').\n"
            "6. EXPLICIT SPOKEN DIALOGUE SYNC: EVERY SINGLE spoken word must be written inside speech marks at its exact timestamp (e.g. At 00:02 off-screen child voice narrates \"Red!\", At 00:05 off-screen child voice narrates \"Blue!\").\n"
            "7. REALISTIC TIMING BUDGET: In a 10-second video, limit interactive items to 2 to 3 items max so each item gets 3-4 seconds for visual pop and narration sync.\n"
            "8. Mascot SFX & Mouth Opening Sync: Mascots NEVER speak human words. Whenever the mascot makes an animal sound effect (woof/bark/meow/squeak), the prompt MUST explicitly detail the visual mouth opening action at that exact timestamp (e.g. 'At 00:09, Dookie opens his mouth wide and barks happy puppy woofs, mouth movement and barking sound synchronized frame-by-frame. NO mascot human speech.').\n"
    ...
    def _build_user_prompt(self, topic: str, mascot: str) -> str:
        ...
        return (
            ...
            '  "animation_prompt": "Single continuous one-shot video clip, Masterpiece 3D Pixar feature-film animation quality, Octane Render 3D aesthetic, Medium 35mm lens, eye-level, slow push in, warm studio key lighting, soft rim lighting highlights along coat, subsurface scattering (SSS) shader, soft depth of field — ' + mascot_cap + ', the mascot character from image 1 and image 2, sits happily in a bright playroom with a sunshine yellow background (#FFD166). At 00:02, a glowing 3D red ball pops up beside ' + mascot_pronoun + ', off-screen child voice narrates \\"Red!\\". At 00:05, a glowing 3D blue ball pops up, off-screen child voice narrates \\"Blue!\\". At 00:08, a glowing 3D pink ball pops up, off-screen child voice narrates \\"Pink!\\". ' + mascot_cap + ' opens mouth wide at 00:09 and makes ' + sfx_desc + ', mouth movement and sound synchronized frame-by-frame. NO on-screen text, NO subtitles, NO AI captions.\\n\\nLocation: Bright colorful playroom, sunny morning.\\nAudio: Off-screen friendly child narrator voice matching vocal_reference.mp3, upbeat cheerful children\'s background music matching music_reference.mp3, mascot ' + sfx_desc + ' (NO mascot human speech).",\n'
            '  "narration_text": "Red! Blue! Pink!",\n'
            '  "subtitles": [\n'
            '    {"start": 0.0, "end": 4.0, "text": "Red!"},\n'
            '    {"start": 4.0, "end": 7.0, "text": "Blue!"},\n'
            '    {"start": 7.0, "end": 10.0, "text": "Pink!"}\n'
            '  ]\n'
            "}"
        )
