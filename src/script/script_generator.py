...
class ScriptGenerator:
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "Rule 8: Visual & Audio Event Synchronization: When the scene demonstrates sequential actions or items (e.g. counting numbers, pops, shapes, colors), animation_prompt MUST explicitly detail the exact sync between the visual event pop on screen and the spoken narrator word (e.g. 'As number 1 pops up, narrator speaks 'One!'. As number 2 pops up, narrator speaks 'Two!'. Visual pops and spoken words are synchronized frame-by-frame.').\n"
            "Rule 9: Output valid JSON ONLY adhering to the requested schema."
        )
    ...
    def _build_user_prompt(self, topic: str, mascot: str) -> str:
        ...
        return (
            ...
            "    {\n"
            '      "scene_number": 1,\n'
            '      "animation_prompt": "Shot 1: ' + mascot_cap + ', the mascot character from image 1 and image 2, is sitting happily in a bright colorful room with a sunshine yellow background (#FFD166). As 3D elements for ' + clean_topic + ' pop up on screen one by one beside ' + mascot_cap + ', off-screen friendly child narrator voice speaks clearly in sync with each visual pop. Background Music: Upbeat cheerful children\'s background music playing consistently throughout. Mascot SFX: ' + mascot_cap + ' makes ' + sfx_desc + '. Smooth 3D Pixar animation, cute 3D character style.",\n'
        ...
        if mock_mode or not self.api_key:
            ...
            mock_script = ShortScript(
                ...
                scenes=[
                    ScenePrompt(
                        scene_number=1,
                        animation_prompt=f"Shot 1: {mascot_cap}, the mascot character from image 1 and image 2, is sitting happily in a bright colorful room with a sunshine yellow background (#FFD166). As 3D elements for {clean_topic} pop up one by one, off-screen friendly child narrator voice speaks in sync with each visual pop. Background Music: Upbeat cheerful children's background music playing consistently throughout. Mascot SFX: {mascot_cap} makes {sfx_desc}. Smooth 3D Pixar animation, cute 3D character style.",
        ...
        try:
            ...
            if not scenes:
                scenes = [
                    ScenePrompt(
                        scene_number=1,
                        animation_prompt=f"Shot 1: {mascot_cap}, the mascot character from image 1 and image 2, is dancing happily and teaching {clean_topic}. Audio: Off-screen friendly child voice narrates in sync with visual pops. Background Music: Upbeat cheerful children's background music. Mascot SFX: {mascot_cap} makes {sfx_desc}. Smooth 3D Pixar animation, cute 3D character style.",
