...
Uses the Multi-Shot Animation Prompt Framework for Segmind Seedance 2.0.
"""
...
class ScriptGenerator:
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "5. Audio & Dialogue Sync: Spoken narration is ONLY delivered by an off-screen friendly child voice. Place exact spoken lines inside speech marks in the shot where it lands (e.g. off-screen child voice narrates \"Look! One red ball!\"). Explicitly detail frame-by-frame sync between 3D visual pops and spoken words.\n"
            "6. Mascot SFX: Mascots NEVER speak human words. Dookie (puppy barks/woofs), Mia (meows/purrs), Carrot (squeaks/crunch).\n"
            "7. NO AI Text on Screen: Mandate inside prompt: 'NO on-screen text, NO subtitles, NO captions generated by AI. Clean 3D video frames only.'\n"
            "8. Metadata Footer: Prompt MUST end with:\n"
            ...
            "9. Prompt Length Limit: Entire animation_prompt MUST be 1,500 characters or fewer.\n"
            "10. Metadata Rules: Title < 100 chars (NO '| Dookie Tv', max 2 hashtags). Description with #kids #shorts #dookietv.\n"
            "11. Output valid JSON ONLY adhering to the requested schema."
        )
    ...
    def _build_user_prompt(self, topic: str, mascot: str) -> str:
        ...
        return (
            f"Generate a multi-shot Short script (15.0 seconds total) featuring mascot '{mascot_cap}' teaching topic '{clean_topic}'.\n"
            ...
            '  "animation_prompt": "[00:00 - 00:05] Wide, 24mm, eye-level, track — ' + mascot_cap + ', the mascot character from image 1 and image 2, sits happily in a bright playroom with a sunshine yellow background (#FFD166). As a 3D element for ' + clean_topic + ' pops up, off-screen child voice narrates \\"Look! ' + mascot_cap + ' learns ' + clean_topic + '!\\". ' + mascot_cap + ' waves with a big smile.\\n\\n[00:05 - 00:10] Medium, 50mm, high angle, push in — A sky blue 3D element pops up next, off-screen child voice narrates \\"Count with ' + mascot_cap + '!\\". ' + mascot_cap + ' tilts head playfully.\\n\\n[00:10 - 00:15] Close-up, 85mm, low angle, static — ' + mascot_cap + ' smiles as 3D elements bounce rhythmically. NO on-screen text, NO subtitles, NO AI captions.\\n\\nLocation: Bright colorful playroom, sunny morning.\\nAudio: Off-screen friendly child narrator voice, upbeat cheerful children\'s background music, mascot ' + sfx_desc + ' (NO mascot human speech).",\n'
            '  "narration_text": "Look! ' + mascot_cap + ' learns ' + clean_topic + '! Count with ' + mascot_cap + '!",\n'
            '  "subtitles": [\n'
            '    {"start": 0.0, "end": 5.0, "text": "Look! ' + mascot_cap + ' learns ' + clean_topic + '!"},\n'
            '    {"start": 5.0, "end": 10.0, "text": "Count with ' + mascot_cap + '!"},\n'
            '    {"start": 10.0, "end": 15.0, "text": "Great job!"}\n'
            '  ]\n'
            "}"
        )
    ...
    def generate_script(self, topic: str, mascot: str, mock_mode: bool = True) -> Tuple[ShortScript, float]:
        ...
        if mock_mode or not self.api_key:
            ...
            mock_prompt = (
                ...
                f"sits happily in a bright playroom with a sunshine yellow background (#FFD166). As a glowing 3D element for {clean_topic} pops up, "
                f'off-screen child voice narrates "Look! {mascot_cap} learns {clean_topic}!". {mascot_cap} waves excitedly.\n\n'
                f"[00:05 - 00:10] Medium, 50mm, high angle, push in — A sky blue 3D element pops up next, "
                f'off-screen child voice narrates "Count with {mascot_cap}!". {mascot_cap} tilts head playfully.\n\n'
                f"[00:10 - 00:15] Close-up, 85mm, low angle, static — {mascot_cap} smiles bright as 3D elements bounce around. "
            ...
            mock_script = ShortScript(
                ...
                narration_text=f"Look! {mascot_cap} learns {clean_topic}! Count with {mascot_cap}!",
                subtitles=[
                    SubtitleItem(start=0.0, end=5.0, text=f"Look! {mascot_cap} learns {clean_topic}!"),
                    SubtitleItem(start=5.0, end=10.0, text=f"Count with {mascot_cap}!"),
                    SubtitleItem(start=10.0, end=15.0, text="Great job!"),
                ],
                total_duration_seconds=15.0,
            )
